...
async function generateBukuKegiatanInsight(eventId, b, hash){
  ...
  try{
    const teks = await aiTanya(buatPromptInsightBukuKegiatan(b), {
      system: 'Kamu asisten keuangan untuk aplikasi kas Karang Taruna. Tulis ringkasan kondisi kas dalam Bahasa Indonesia yang netral dan lugas, gaya laporan singkat — BUKAN sapaan akrab (jangan pakai "teman-teman", "kita", "yuk", atau sapaan sejenis; gunakan sudut pandang orang ketiga/netral, mis. "kas saat ini...", "iuran tercatat..."). 3-5 kalimat, boleh pakai 1 kalimat penutup berupa saran praktis kalau memang relevan (tidak wajib dipaksakan tiap kali). Kalau datanya ada anggota belum bayar iuran atau agenda kegiatan dalam 7 hari ke depan, boleh disinggung singkat kalau relevan dengan kondisi kas (jangan sebut nama anggota, cukup jumlah). Jangan mengulang semua angka yang sudah tampil di layar — pilih yang paling penting untuk diberi konteks/makna. Jangan pakai heading, bullet, atau markdown, cukup paragraf biasa.',
    });
