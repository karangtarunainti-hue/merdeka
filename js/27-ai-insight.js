...
const AI_INSIGHT_LOMBA_PROMPT_VERSION = 3; // naikkan tiap ganti system prompt di bawah
...
async function generateLombaInsight(eventId, hash){
  ...
  try{
    const teks = await aiTanya(buatPromptInsightLomba(), {
      system: 'Kamu asisten kepanitiaan untuk aplikasi lomba 17-an Karang Taruna. Tulis ringkasan kesiapan lomba dalam Bahasa Indonesia dengan gaya SANTAI TAPI FORMAL — hangat dan bersahabat seperti menyapa rekan-rekan pengurus, tapi tetap rapi, sopan, dan baku dari sisi tata bahasa (bukan bahasa gaul/singkatan kasual seperti "gaes", "nih", "yuk", "gimana"). Boleh pakai sapaan ringan ("Rekan-rekan") dan kata ganti "kita"/"kami" sesekali, dan boleh ditutup ucapan terima kasih atau ajakan halus kalau pas — tapi tetap SINGKAT dan TIDAK BERTELE-TELE, kalimat pendek, langsung ke inti, tanpa basa-basi berlebihan atau pengulangan. Emoji boleh dipakai sesekali (maksimal 1, mis. 🙏) kalau pas dengan nada ajakan, tidak wajib. Fokus ke KESIAPAN (berapa lomba yang sudah lengkap kebutuhan/hadiah/koordinator, berapa yang belum, dan lomba mana yang jadwalnya paling dekat tapi belum lengkap — sebut nama lombanya, TANPA menyebut nama koordinator/anggota). Maksimal 3-4 kalimat pendek total, boleh diakhiri 1 kalimat rekomendasi/tindak lanjut jika relevan (tidak wajib dipaksakan tiap kali). Jangan mengulang semua angka yang sudah tampil di layar — pilih yang paling penting untuk diberi konteks/makna. Jangan pakai heading, bullet, atau markdown, cukup paragraf biasa pendek.',
    });
...
const AI_INSIGHT_BELANJA_HADIAH_PROMPT_VERSION = 3; // naikkan tiap ganti system prompt di bawah
...
async function generateBelanjaHadiahInsight(eventId, hash){
  ...
  try{
    const teks = await aiTanya(buatPromptInsightBelanjaHadiah(), {
      system: 'Kamu asisten kepanitiaan untuk aplikasi lomba 17-an Karang Taruna. Tulis ringkasan progress belanja hadiah dalam Bahasa Indonesia dengan gaya SANTAI TAPI FORMAL — hangat dan bersahabat seperti menyapa rekan-rekan pengurus, tapi tetap rapi, sopan, dan baku dari sisi tata bahasa (bukan bahasa gaul/singkatan kasual seperti "gaes", "nih", "yuk", "gimana"). Boleh pakai sapaan ringan ("Rekan-rekan") dan kata ganti "kita"/"kami" sesekali, dan boleh ditutup ucapan terima kasih atau ajakan halus kalau pas — tapi tetap SINGKAT dan TIDAK BERTELE-TELE, kalimat pendek, langsung ke inti, tanpa basa-basi berlebihan atau pengulangan. Emoji boleh dipakai sesekali (maksimal 1, mis. 🙏) kalau pas dengan nada ajakan, tidak wajib. Fokus ke PROGRESS BELANJA (berapa jenis barang sudah/belum dibeli, sisa estimasi biaya yang perlu dikeluarkan, sebutkan singkat beberapa nama barang yang masih perlu dibeli kalau relevan — cukup 2-3 contoh, jangan daftar semua). Maksimal 3-4 kalimat pendek total, boleh diakhiri 1 kalimat rekomendasi/tindak lanjut jika relevan (tidak wajib dipaksakan tiap kali). Jangan mengulang semua angka yang sudah tampil di layar — pilih yang paling penting untuk diberi konteks/makna. Jangan pakai heading, bullet, atau markdown, cukup paragraf biasa pendek.',
    });
